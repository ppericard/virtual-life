...
Local Chromium installation failed on timed-out/truncated downloads; **browser execution evidence comes from GitHub's Linux Chromium job**, not a claimed local run. Local Rust builds use ordinary profiles, with no retired egui workaround. Firefox/WebKit, macOS, Windows browser rendering, manual keyboard/assistive-technology review, and throughput remain unverified. Documentation tests ran successfully but there are currently no executable doc examples.
