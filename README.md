...
**6 September 2026 — Task 02 browser demonstrator, under verification.** Pierre chose a local browser GUI to replace the native viewer. The native Rust engine, synchronous transitions, five-tick script, integer domains, and headless executable remain. The native frontend is retained only during this replacement's parity checks, then removed; its history remains in PR #2. This is scripted behavior, not emergence or an accepted autonomous model.
...
**Current evidence:** local Linux headless debug/release tests (29), web-enabled tests (35), frontend syntax/two display tests, and Clippy have passed during implementation. npm audit reports no known advisories for the locked test dependencies. Browser installation encountered download timeouts in this workspace; real browser automation, screenshot inspection, RustSec findings, and final CI results remain pending. Final tested SHAs and run links will be recorded in the PR after execution. A workflow file alone is not CI success.

CI has stable Quality (Linux), Tests (Linux), Tests (Windows), Browser (Linux Chromium), and Dependency audit jobs, with timeouts and superseded-run cancellation. Actions use verified full commit SHAs; permissions are `contents: read`, without privileged PR triggers. The browser job keeps its HTML report, known-state screenshots, and failure traces/screenshots as a 14-day artifact. There is no arbitrary coverage/performance gate. No browser-engine/platform matrix beyond Linux Chromium is claimed.
...
Browser refresh loses page-local observation history and selection. Multiple pages share one experiment and controls; there is no multi-user coordination. Abrupt process termination loses the run; Ctrl+C is the supported clean shutdown. Firefox, WebKit, macOS, Windows browser rendering, throughput, and assistive-technology behavior need separate verification.
