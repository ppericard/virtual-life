...
The whole world fits below compact run controls on desktop, with square cells and exact total/group counts. On narrow screens the world appears directly below the controls. Click an occupied square or choose **Inspect** to follow an agent in a dismissible panel; closing it keeps the selection. Wear-repair inspection shows the last selected action (which may have failed), or “Not yet acted” for initial agents and newborns. This memory currently leaves action probabilities unchanged.
