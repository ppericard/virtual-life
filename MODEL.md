...
**10 September 2026 — browser property presets (Pierre's decision):** Original, Moderate movement, Wide movement range and Lower copying are optional wear-repair initialization choices. Selecting a button changes only the page's pending choice; an explicit fixed/random-seed restart applies it. A preset replaces the four weight bundles and sets their proportions to equal shares, preserving dimensions, occupancy, maintenance, duration, pacing and sampling. Seed-only restarts subsequently replay those current effective settings. Pending choices survive polling and same-run reconnect; a new run synchronizes them to its actual settings. Current custom settings remain available when cancelling a pending named choice. A failed replacement retains both the old run and its settings. Demo and random-removal runs do not accept these presets. Preset names describe illustrative property combinations, not inherited identities or biological classes, and do not guarantee coexistence. Group identity and inheritance still come from actual properties; no transition rule or default changes.
