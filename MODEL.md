...
**Acquisition decision — 24 September 2026:** Pierre chose a separate **Take** unit action over Repair drawing material from a neighbour. Take becomes the fifth automaton state (Wait, Move, Copy, Repair, Take): it transfers loose material held by another local agent into the actor's reserve. Repair only converts the actor's own reserve into structure. In the material lab (a disposable prototype, removed once these decisions were made; see the history of PR #21), a Repair that pulls only its immediate shortfall always empties reserve, so Copy's reserve requirement could never be met again; a separate Take keeps one transformation per action and lets graphs accumulate reserve before copying. Target choice, transfer amount, access to loose material held by active agents and competing Take claims remain open. The five-state graph will be a new automaton protocol; the Rust engine does not implement it yet.

**Structural requirements — 24 September 2026:** Pierre chose that Take requires at least 1 structure, while Repair has no structural requirement. Move and Copy already need structure after their attempt wear. Consequences: an agent with zero structure but some reserve can still Repair and resume activity; an agent with zero structure and zero reserve can only Wait, while others may Take its loose material until its empty record is removed; separated fragments holding only loose material stay passive. These conditions are observations derived from material and the common action requirements, not stored types or flags. Whether Take also costs attempt wear is left to quantitative tuning.

**Access to loose material — 24 September 2026:** Pierre chose that loose material leaves its holder only when another agent Takes it. Take may target the loose material of any local agent, whether it is active, dormant or spent; an agent never takes back its own loose material, and there is no separation action. Recycling one's own wear alone is therefore impossible, while neighbours can clear each other's loose material. Restricting Take to spent agents was rejected: wear would accumulate in every active agent until its cell saturates, a disguised countdown. Whether a pair of agents exchanging loose material can sustain itself without other resources depends on quantitative tuning and should be measured, not assumed.

**Occupancy — 24 September 2026:** Pierre chose to keep one agent per cell for the first material implementation. Stocks and remains occupy cells like any other agent and are used from neighbouring cells through Take; a cell becomes free once others have taken all the material of the agent holding it. Copy still needs an empty neighbouring cell, and crowding still counts occupied neighbours whatever their condition. The lab's per-cell material capacity becomes a per-agent material limit, to be tuned. Pierre's caveat: stacking must still be considered later, if only because static resources may block space. The first material runs should show how much space stocks and remains take up.

**Irreversible failure — 24 September 2026:** with the decisions above, an agent with zero structure and zero reserve is spent for good. Structure rises only through the agent's own Repair, which needs reserve, and reserve rises only through its own Take, which needs structure. A spent agent can therefore only Wait: its attempts to Move, Copy, Take or Repair have no effect, its loose material only decreases as others Take it, and its record is removed once empty. If nobody takes that material, the agent remains a static holder indefinitely. Zero structure with some reserve is dormancy, not failure: Repair can restart the agent. Two invariants protect this outcome and must hold in every implementation:

1. **No action adds material to another existing agent.** Take moves material toward the actor and Copy funds a new agent. Any future transfer toward an existing agent, such as a Give action, would let spent agents recover and needs an explicit decision.
2. **Resource stocks start with loose material only**, without structure or reserve, and with graphs whose arrows all lead to Wait.

Every active agent therefore descends through Copy from an initially active agent; stocks and remains never become active. The Rust implementation must test both invariants with generated runs: spent agents never regain structure or reserve, and stocks never gain either. The material lab checked this over 120,000 random actions in 400 worlds: no spent agent or stock gained structure or reserve, while 214 dormant agents restarted through Repair. Inspection should show the derived condition (active, dormant or spent) alongside attempted actions and their outcomes, so a spent agent's failed attempts are not read as activity.
...
**Shared cells are deferred, not rejected.** Pierre suggested allowing agents to stack on cells so individuals, resources and remains can coexist locally. The first material implementation keeps one agent per cell (see the occupancy decision above), but stacking remains a planned option, particularly if static resources or remains block space. Any shared-cell design must use the same occupancy rules for every agent, not separate biological/resource layers. Capacity, crowding, targeting, simultaneous claims and inspection would need revised rules; the current engine still permits one agent per cell.
