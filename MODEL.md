...
**Acquisition decision — 24 September 2026:** Pierre chose a separate **Take** unit action over Repair drawing material from a neighbour. Take becomes the fifth automaton state (Wait, Move, Copy, Repair, Take): it transfers loose material held by another local agent into the actor's reserve. Repair only converts the actor's own reserve into structure. In the material lab, a Repair that pulls only its immediate shortfall always empties reserve, so Copy's reserve requirement could never be met again; a separate Take keeps one transformation per action and lets graphs accumulate reserve before copying. Target choice, transfer amount, access to loose material held by active agents and competing Take claims remain open. The five-state graph will be a new automaton protocol; the Rust engine does not implement it yet.
