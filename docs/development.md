...
Both policies share SplitMix64 and `VirtualLife sampling v1` initialization/bounded draws. The default **wear-repair crowding v2** scales Copy by the fraction of eight neighbours empty at the start and transfers the removed share to Wait. For base `(W,M,C,R)` and empty count `E`, use `u64` tickets `(8W+(8-E)C, 8M, EC, 8R)`. Their total is at most `32 × u32::MAX`; odd weights stay exact without division or rounding. Temporary tickets never change inherited properties or groups. The browser labels base weights and describes crowding alongside the existing protocol metadata.

Wear-repair scans starting occupied squares in row-major order, skips all draws for individuals unable to survive upkeep, then draws below `8(W+M+C+R)` once per survivor. Always keep this bound, including eight empty neighbours and zero Copy. Neighbours about to fail remain occupied for the calculation. A selected Move/Copy then draws below 8 among **all** neighbours before affordability, including occupied targets; no retry. Counting occupancy and adjusting tickets add no RNG calls; bounded rejection, costs, repair and newborn scheduling retain their documented behavior. **Wear v2 seeds do not replay wear-repair v1 trajectories**, even if probabilities coincide, because the action bound changed. Initialization and **random v1** bounds/draw order are unchanged. Effective settings and protocol appear in headless/browser output; retain the source commit and Cargo.lock for reproduction.
