...
The World canvas uses the full content width, with square cell spacing and interiors, responsive panels below and a bounded-width plot. Its aspect ratio follows the world dimensions plus a slim, even frame; there are no numbered axes. Exact zero-based coordinates remain in agent inspection. Drawing and hit testing share one cell pitch and a uniform CSS scale, keeping letters and selection borders undistorted. The longest logical grid edge is 540 units, with an 8-unit frame on every side. Backing stores follow displayed size and device pixel ratio, with the World raster capped at 4096 pixels per side (at most 16,777,216 pixels). Beyond that cap the raster uses a lower resolution without changing cell shape or hit testing; extremely elongated worlds can have cells too small to distinguish. Plot geometry is unchanged. Resizing repaints retained observations, including while paused or disconnected, without adding history, rebuilding the agent selector or sending controls.
...
Wear-repair mode offers four preset buttons next to the restart controls. The current-run label and next-restart choice are separate. Selection alone sends no request and changes no observed world or legend. Pending seed/preset edits survive polling and same-run reconnect; a new run resets them to its actual settings. For custom settings, **Keep current custom settings** cancels a pending named choice before restart. Once a preset is applied, it becomes the current configuration; the previous custom launch configuration is not a saved slot. Preset choices lock with the other mutation controls during an outstanding command or disconnection. Demo and random-removal modes hide the preset choices.
